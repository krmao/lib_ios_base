...
+ (NSInteger)toInt:(NSString *)s {
    ...
    bool bo = [STStringUtil isNumString:s];
...
+ (double)toDouble:(NSString *)s {
    ...
    bool valid = [STStringUtil isValidDigitString:s];
...
+ (bool)isValidPostCode:(NSString *)code {
    if (code == nil || [code isEqualToString:@""] || code.length != 6 || ![STStringUtil isNumString:code]) {
        return false;
    }
...
+ (bool)isMobileNumber:(NSString *)mobile {
    ...
    if (mobile != nil && mobile.length == 11) {
        ...
        //        if([STStringUtil isNumString:firstStr] && [STStringUtil isNumString:mobile] && [firstStr integerValue] == 1)
        //        {
        //            isMobile = true;
        //        }
        // 马惠修改，第一个数字可以不是1
        if ([STStringUtil isNumString:mobile]) {
            isMobile = true;
        }
    }
...
+ (bool)isMobileNumber2:(NSString *)mobile {
    ...
    if (mobile != nil && mobile.length == 11) {
        ...
        if ([STStringUtil isNumString:firstStr]
                && [STStringUtil isNumString:mobile]
...
+ (bool)verifyID:(NSString *)idcard {
    ...
    if ([verify isEqualToString:[STStringUtil getVerify:idcard]]) {
        return true;
    }
...
+ (NSMutableArray *)separatedByDot:(NSString *)str Dot:(NSString *)dot {
    ...
    if ([STStringUtil emptyOrNull:str] || [STStringUtil emptyOrNull:dot]) {
    ...
    for (unsigned int i = 0; i < [arrayByDot count]; i++) {
        ...
        if (![STStringUtil emptyOrNull:stringItem]) {
...
+ (NSString *)convertRMBDisplayString:(long long)rmbCents {
    return [STStringUtil convertRMBDisplayStringByLong:rmbCents];
}
